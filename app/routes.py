...
def initiate_action_route():
    ...
    target_type = request.form.get('target_type', 'institution') # institution atau manual_url
    
    # Variabel untuk menyimpan tag dan method yang akan digunakan
    final_institution_tag = None
    final_crawl_method = None

    # Parameter crawling default
    default_max_depth = current_app.config.get('MAX_DEPTH_LIMIT_DEFAULT', 2)
    default_max_pages = current_app.config.get('MAX_PAGES_DEFAULT', 50)
    ...
    if target_type == 'institution':
        selected_db_file = request.form.get('selected_db_file') # Ini dari dropdown baru
        ...
        if not keyword: 
             ...
             # Redirect kembali dengan db_target agar dropdown tetap terpilih
             return redirect(url_for('main.index', db_target=selected_db_file))
        # Untuk 'institution', parameter crawl (depth, pages, clear) tidak diambil dari form
        # karena hanya akan melakukan search.

    elif target_type == 'manual_url':
        manual_seed_url_form = request.form.get('manual_seed_url', '').strip()
        final_crawl_method = request.form.get('crawl_method', 'bfs') # Ambil metode dari form
        clear_data_flag = request.form.get('clear_data_on_search_crawl') == 'yes'

        if not manual_seed_url_form:
            flash("Seed URL manual baru wajib diisi.", "danger")
            return redirect(url_for('main.index', keyword=keyword))
        try:
            ...
            # Generate institution tag dari URL manual
        ...
        try: # Ambil parameter crawl dari form
            ...
            max_pages_from_form = int(max_pages_str) if max_pages_str.strip() else default_max_pages
        except ValueError:
            flash("Nilai kedalaman atau maks halaman harus berupa angka.", "danger")
            return redirect(url_for('main.index', keyword=keyword, manual_seed_url=manual_seed_url_form))

        if not keyword and max_pages_from_form <= 0:
            flash("Untuk URL Manual Baru, masukkan kata kunci atau pastikan Maks Halaman > 0.", "danger")
            return redirect(url_for('main.index', keyword=keyword, manual_seed_url=manual_seed_url_form))
        
        if not (0 <= max_depth_from_form <= 5): 
            flash(f"Kedalaman maksimal (untuk crawling) harus antara 0 dan 5.", "danger")
            return redirect(url_for('main.index', keyword=keyword, manual_seed_url=manual_seed_url_form))
        if not (max_pages_from_form == 0 or (1 <= max_pages_from_form <= 500)):
            flash(f"Maksimal halaman (untuk crawling) harus 0 (tidak crawl) atau antara 1 dan 500.", "danger")
            return redirect(url_for('main.index', keyword=keyword, manual_seed_url=manual_seed_url_form))

    else: # Tipe target tidak dikenal
    ...
    session['action_params'] = {
        'keyword': keyword,
        'target_type': target_type, # Untuk membedakan logika di process_crawling
        'institution_tag': final_institution_tag, # Selalu diisi
        'crawl_method': final_crawl_method,       # Selalu diisi
        # Parameter di bawah ini relevan utama untuk manual_url dengan crawling
        'manual_seed_url': request.form.get('manual_seed_url', '').strip() if target_type == 'manual_url' else None,
...
def process_crawling_route():
    ...
    target_type = action_params.get('target_type') # 'institution' atau 'manual_url'
    
    # institution_tag dan crawl_method sekarang selalu ada di action_params
    current_institution_tag_for_db = action_params.get('institution_tag')
    crawl_method = action_params.get('crawl_method','bfs') 

    manual_seed_url_input = action_params.get('manual_seed_url') # Bisa None jika target_type 'institution'
    max_depth_to_crawl = action_params.get('max_depth', current_app.config.get('MAX_DEPTH_LIMIT_DEFAULT', 2))
    max_pages_to_crawl = action_params.get('max_pages', current_app.config.get('MAX_PAGES_DEFAULT', 50))
    ...
    # Logika inisialisasi DB / clear data (disempurnakan)
    if target_type == 'manual_url' and current_seed_url and clear_data_before_crawl and db_existed_before_init:
        try:
            ...
            self_log_for_template.append(f"Data lama dari '{g.current_db_name}' telah dihapus (clear data).")
            db_existed_before_init = False # Anggap DB baru akan dibuat
        except Exception as e_del:
            self_log_for_template.append(f"GAGAL hapus data lama '{g.current_db_name}': {e_del}")
    ...
    if not db_existed_before_init and ( (target_type == 'manual_url' and current_seed_url and max_pages_to_crawl > 0) or keyword ): # Hanya init jika akan crawl atau ada keyword
        if current_app.config.get('AUTO_INIT_DB', True):
            ...
            g.current_db_name = None # Tidak ada DB yang bisa dipakai
    ...
    if current_seed_url and max_pages_to_crawl > 0 and target_type == 'manual_url': # Crawl hanya untuk manual_url
        if not g.current_db_name or not os.path.exists(db_path_for_action):
            ...
            if crawler_instance:
                ...
                if pages_added == 1 and db_was_newly_created_this_request:
    ...
    return render_template('proses_crawling.html',
                           ...
                           crawl_log=self_log_for_template,
                           crawl_summary=crawl_summary,
                           redirect_url=url_for('main.show_search_results_route'),
                           redirect_url_tree=redirect_url_tree, 
                           db_name_used=(g.current_db_name if g.current_db_name and os.path.exists(db_path_for_action or "") else "Tidak ada DB aktif/valid")
                           )
